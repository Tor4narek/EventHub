  ...
  return <div className="overlay" onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
    <div className="drawer" role="dialog" aria-modal="true" aria-label={event ? 'Редактирование мероприятия' : 'Новое мероприятие'}>
      ...
      <form onSubmit={submit} className="drawer-body form-stack">
        <label>Название <span className="required">*</span><Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Например, лекция о городском искусстве" required /></label>
        <label>Описание <span className="required">*</span><Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="О чём мероприятие и кому оно будет интересно" required rows={5} /></label>
        <div className="form-grid"><label>Дата и время · МСК <span className="required">*</span><input className="native-input" type="datetime-local" value={date} onChange={e => setDate(e.target.value)} required /></label><label>Дедлайн · МСК<input className="native-input" type="datetime-local" value={deadline} onChange={e => setDeadline(e.target.value)} /></label></div>
        <label>Место <span className="required">*</span><Input value={locationText} onChange={e => setLocationText(e.target.value)} placeholder="Адрес, площадка или Онлайн" required /></label>
        <label>Первоисточник / регистрация <span className="required">*</span><Input type="url" value={source} onChange={e => setSource(e.target.value)} placeholder="https://..." required /></label>
